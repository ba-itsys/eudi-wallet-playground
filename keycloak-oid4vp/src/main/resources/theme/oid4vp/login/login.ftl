        ...
        <div id="kc-form">
          <div id="kc-form-wrapper">
            <#if realm.password>
                ...
                <#assign oid4vpExecution = "">
                <#if auth?? && auth.authenticationSelections??>
                    <#list auth.authenticationSelections as authenticationSelection>
                        <#if authenticationSelection.authenticationExecution?? && authenticationSelection.authenticationExecution.authenticator?? && authenticationSelection.authenticationExecution.authenticator == "oid4vp-login">
                            <#assign oid4vpExecution = authenticationSelection.authExecId>
                        </#if>
                    </#list>
                </#if>
                <#if oid4vpExecution?has_content>
                    <form id="kc-oid4vp-login-form" action="${url.loginAction}" method="post" novalidate="novalidate">
                        <input type="hidden" name="authenticationExecution" value="${oid4vpExecution}"/>
                        <button id="kc-oid4vp-login"
                                type="submit"
                                class="${properties.kcButtonClass!} ${properties.kcButtonSecondaryClass!} ${properties.kcButtonDefaultClass!} ${properties.kcButtonBlockClass!} ${properties.kcButtonLargeClass!}">
                            Login with OID4VP
                        </button>
                    </form>
                </#if>
